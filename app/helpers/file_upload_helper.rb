  ...
  def upload_image(data, path:, filename: nil)
    ...
    begin
      ...
      if file.size > 1.megabyte
        puts "[ERROR] FileUploadHelper.upload_image - file is too large for upload: #{file.size/1024} KB"
      else
        puts "FileUploadHelper.upload_image - uploading image of size: #{file.size/1024} KB"
        ...
        puts "FileUploadHelper.upload_image - uploaded to #{obj.public_url}"
        url = obj.public_url
      end
    ensure
      file.close
